<?php
    class Data extends Model{
        ...
        public static function selectDiagram($date, $data){
            $strings=array();
            
            $dates[] = $date;
            $x = 1;
            while ($x <= 5) {
                $dates[] = date('Y-m-d', strtotime("$date - ".(7*$x)." days"));
                $x++;
            }
            
            $id_station = array_keys($data);
            $records=Model::factory('Data')->where_in('date',$dates)->where_in('id_station',$id_station)->orderByAsc('id_station')->orderByAsc('date')->findMany(); 
            $i = 0;
            $tmpId = 0;
            foreach($records as $record){
                $id = $record->id_station;

                if($tmpId < $id) { $i = 0; }
                $tmpId = $id;

                $data[$id]['date'][$i] = date('d.m', strtotime($record->date));
                $data[$id]['uvb'][$i] = $record->uvb;
                $i++;
            }
            return $data;
        }
