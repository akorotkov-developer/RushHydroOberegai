...
class _ets
{
    ...
    {
        if (error_reporting() & $error_level || true) {
            switch ($error_level) {
                case E_NOTICE:
                    $type = 'notice';
                    break;
    ...
    function elt_label($eltid)
    {
        switch ($eltid) {
            ...
            case _ETS_TEXT:
                return 'text element';
            case _ETS_TAG:
                return 'simple tag element';
            case _ETS_ALT_TAG:
                return 'alternate tag element';
            case _ETS_TEMPLATE:
                return 'template element';
            case _ETS_SET:
                return 'set element';
            case _ETS_SETVAL:
                return 'set-value element';
            case _ETS_MIS:
            ...
            case _ETS_IF:
                return 'if element';
            case _ETS_CODE:
                return 'PHP code or test';
            case _ETS_CHOOSE:
                return 'choose element';
            case _ETS_WHENTEST:
                return 'when-test element';
            case _ETS_ELSE:
                return 'else element';
            case _ETS_CHOOSEVAR:
                return 'choose-variable element';
            case _ETS_WHENVAL:
                return 'when-value element';
            case _ETS_CALL:
                return 'call element';
            case _ETS_ARG:
                return 'argument element';
            case _ETS_MIS_TEMPLATE:
                return 'missing template element';
            case _ETS_REDUCE:
            ...
            case _ETS_EVAL:
                return 'eval element';
            case _ETS_SAFE:
                return 'safe eval element';
            case _ETS_ROOT_EVAL:
                return 'eval or safe element';
        }
    }
    ...
    function parse($ptype, &$i, &$line, $ncontent, $content)
    {
        ...
        for (; $i < $ncontent; ++$i) {
            ...
            $c0 = mb_substr($content, $i, 1);
            $c1 = mb_substr($content, $i + 1, 1);
            ...
            if ($mode == _ETS_DATA) {
                ...
                if ($c0 == '{') {
                    ...
                    $c2 = mb_substr($content, $i + 2, 1);
                    $c3 = mb_substr($content, $i + 3, 1);
                    $c4 = mb_substr($content, $i + 4, 1);
                    $c5 = mb_substr($content, $i + 5, 1);
                    $c6 = mb_substr($content, $i + 6, 1);
                    $c7 = mb_substr($content, $i + 7, 1);
    ...
    function read_container($container, $parse)
    {
        ...
        if ($parse != _ETS_TEXT) {
            ...
            if ($this->external_cache_read && $this->external_cache_write) {
                ...
                $this->containers[$container] = TRUE;
                $i = 0; $line = 1;
                $masktree = $this->parse($parse, $i , $line , strlen($content), "$content       ");
